<template>
  <div v-if="open" class="fixed inset-0 z-50">
    ...
    <div class="absolute inset-0 grid place-items-center p-4">
      <div class="w-full max-w-md rounded-2xl bg-white dark:bg-slate-800 shadow-soft border border-slate-200 dark:border-slate-700">
        <div class="flex items-start justify-between p-6 border-b border-slate-200 dark:border-slate-700">
          <div>
            <h2 class="text-xl font-semibold text-slate-900 dark:text-slate-100">Acceso para servidores públicos</h2>
            <p class="mt-1 text-sm text-slate-600 dark:text-slate-300">Inicia sesión con tu correo institucional.</p>
          </div>
          <button type="button" @click="emit('close')" class="text-slate-400 dark:text-slate-500 hover:text-slate-600 dark:hover:text-slate-400 flex-shrink-0">
          ...
          <form class="mt-6 space-y-4" @submit.prevent="onSubmit">
            <div>
              ...
              <input id="email" v-model.trim="email" type="email" required
                class="mt-1 block w-full rounded-lg border border-slate-300 dark:border-slate-600 dark:bg-slate-700 dark:text-slate-100 focus:border-primary-500 focus:ring-primary-500" />
            </div>
            <div>
