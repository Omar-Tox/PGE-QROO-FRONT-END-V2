...
// Interfaces usadas por el Dashboard principal
...
// 1. CORRECCIÓN: Inicializar leyendo del storage para no perder sesión al recargar
let authToken: string | null = localStorage.getItem('auth_token')

// --- LÓGICA DE TOKEN ROBUSTA ---

// Helper para obtener token (usado por apiFast)
...
// 2. CORRECCIÓN: Esta función ahora es "inteligente", busca el token si falta.
export function getAuthHeaders(): HeadersInit {
  const token = getStoredToken() // Siempre intenta recuperar el token más fresco
...
async function apiFetch<T>(path: string, options: RequestInit = {}): Promise<T> {
  const headers = getAuthHeaders() // Usa la versión robusta
  ...
  if (!response.ok) {
    ...
    // Si el error viene en JSON (ej. validación 422 o auth 401)
...
// --- OBJETO API PRINCIPAL (Backend Laravel/Django) ---
...
export const api = {
  ...
  async getDependencias(): Promise<Dependencia[]> {
    return safeApiFetch<Dependencia[]>('/dependencias', [])
  },
  ...
  async getDependenciaById(id: string | number): Promise<Dependencia> {
    return safeApiFetch<Dependencia>(`/dependencias/${id}`, { id_dependencia: 0 } as any)
  },
  ...
  async getSectores(): Promise<Sector[]> {
    return safeApiFetch<Sector[]>('/sectores', [])
  },
  ...
  async getEdificiosByDependencia(dependenciaId: string | number): Promise<Edificio[]> {
    return safeApiFetch<Edificio[]>(`/dependencias/${dependenciaId}/edificios`, [])
  },
  ...
  async getPresupuestosByDependencia(dependenciaId: string | number): Promise<any[]> {
    return safeApiFetch<any[]>(`/dependencias/${dependenciaId}/presupuestos`, [])
  },
  ...
  async deletePresupuesto(id: number): Promise<void> {
    await apiFetch(`/presupuestos/${id}`, { method: 'DELETE' })
  },
  ...
  async getUsuarios(): Promise<User[]> {
    return safeApiFetch<User[]>('/usuarios', [])
  },
  ...
  /**
   * DASHBOARD (Con corrección de Fallback)
   */
  async getDashboard(): Promise<DashboardPayload> {
    const fallback = mockDashboard()
    // Si falla (ej. 401 Unauthorized), retorna el fallback seguro para evitar pantalla negra
    return safeApiFetch<DashboardPayload>('/dashboard', fallback)
  }
}

// 3. CORRECCIÓN: Fallback seguro con ceros
